...
class studentFolderController extends Controller
{
    public function studentEditFolderView($student_folder_id)
    {
        ...
        $folders_dropdown = DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', '!=', $student_folder_id)->orWhere('student_subFolder', NULL)->where('active_status', 1)->orderBy('student_folder_id', 'ASC')->get();
    ...
    public function addStudentFolder(Request $request)
    {
        ...
        if ($student_folder_name == NULL) {
            ...
            DB::select('insert into student_note_folder_list (student_folder_name, student_name, student_subFolder, active_status, deleted_date) 
            values (?,?,?,?,?)', [$student_folder_name, $student_name, $student_subFolder, 1, NULL]);
    ...
    public function deleteStudentFolder(Request $request)
    {
        ...
        //Count Rows of Data
        $counter = DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->get();
        $total = count($counter);
        ...
        for ($i = 0; $i <= $total; $i++) {
            ...
            DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', [$delete_id])->update(['deleted_date' => $current_date_time, 'active_status' => 0]);
            //Remove selected notes that is in the deleted folder
            DB::table('student_note_list')->where('student_name', Session::get('username'))->where('student_note_subFolder', [$delete_id])->update(['deleted_date' => $current_date_time, 'active_status' => 0, 'share_status' => 0]);

            $delete_id = DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', [$delete_id])->pluck('student_folder_id')->first();
        }
    ...
    public function recoverStudentFolder(Request $request)
    {
        ...
        for ($i = 0; $i <= $total; $i++) {
            ...
            $recover_current_id = DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', [$recover_id])->pluck('student_folder_id')->first();

            DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', [$recover_id])->update($folder_data);
            DB::table('student_note_list')->where('student_name', Session::get('username'))->where('student_note_subFolder', [$recover_id])->update($note_data);

            $recover_id = DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', [$recover_current_id])->pluck('student_folder_id')->first();

            DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('student_subFolder', [$recover_current_id])->update($folder_data);
            DB::table('student_note_list')->where('student_name', Session::get('username'))->where('student_note_subFolder', [$recover_current_id])->update($note_data);
        }
    ...
    public function studentDeletedFolder()
    {
        if (Session::get('username') == null) {
            ...
            $subjects = DB::table('class_subject_list')->where('class_name', Session::get('user_class'))->orderBy('class_subject_id')->get();
            $announcement = DB::table('announcement_list')->where('annouce_class', Session::get('user_class'))->orderBy('created_at', 'DESC')->get();
            $folders = DB::table('student_note_folder_list')->where('student_name', Session::get('username'))->where('active_status', 0)->orderBy('deleted_date', 'ASC')->get();
